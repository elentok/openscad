...
module grip(grip_depth, object_width, object_height, thingy_depth = 30, bottom_wall_width = 3,
            wall_width = 1.7) {
  ...
  union() {
    ...
    // Fillet Left
    translate([
      0, -large_fillet_radius / 2 - wall_width - object_width / 2, large_fillet_radius / 2 +
      bottom_wall_width
    ]) rotate([ 0, 0, 180 ]) negative_edge(width = grip_depth, radius = large_fillet_radius);
    ...
    translate([ 0, -wall_width / 2 - object_width / 2, object_height / 2 + wall_width / 2 ])
        rotate([ 90, 180, 0 ])
            centered_rounded_edge(width = grip_depth, depth = object_height + wall_width,
